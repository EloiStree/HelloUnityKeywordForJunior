- Unity https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/240 
- Attribute https://github.com/EloiStree/HelloSharpForUnity3D/issues/429   
- Relocation https://github.com/EloiStree/2024_10_19_RelocationRotation
- Faire des boites a outils https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/172
- Bullet Hell Videos https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/170
- Software Install https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/152
- Git Old tutorial https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/81

  
- [ ] [gimbal lock](https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/316)
- [ ] [[HelpURL]](https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/133)
- [ ] [[SelectableBase]](https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/98)
- [ ] InputActionRef 
  - [ ] Vector3
  - [ ] Quaternion
- [ ] IsRight IsFront IsTop
- [ ] IsLeft IsBack IsDown
- [ ] Vector3.SignedAngle
- [ ] Vector3.Angle
- [ ] Vector3.Distance
- [ ] Flat on XY Face, ZY Front, ZX Down
- [ ] Relocate Point
  - [ ] World to Local
  - [ ] Local to World
- [ ] Relocate Quaternion Point
  - [ ] World to Local
  - [ ] Local to World
- [ ] Rotate Point Around Pivot
- [ ] Create Quaternion from 3 Axis 
- [ ] Dot
- [ ] Cross
- [ ] Direction
- [ ] Euler
- [ ] Space.Self Space.World
- [ ] [Triangle 1>2>3](https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/147)
- [ ] VertexColor
- [ ] [ExecuteInEditMode]
- [ ] [RGB](https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/184)
- [ ] Quaternion.Euler(45, 90, 0);
- [ ] Quaternion.Lerp()
- [ ] Quaternion.Inverse()
- [ ] Quaternion.FromToRotation
- [ ] transform.rotation = rotation * transform.rotation;
- [ ] transform.rotation = transform.rotation *  rotation ;

Transform
- [ ] Quaternion.LookAt
- [ ] Quaternion.LookRotation
- [ ] Rotate
- [ ] Translate
- [ ] RotateAround

[Mathf](https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/126)
- [ ] Abs
- [ ] Sign
- [ ] Mathf.Sin, Mathf.Cos, Mathf.Tan
- [ ] Mathf.Atan2
- [ ] Mathf.Random
- [ ] Mathf.Clamp
- [ ] Mathf.Clamp01
- [ ] Mathf.Min , Mathf.Max
- [ ] Mathf.Floor, Mathf.Ceil,
- [ ] Mathf.Round


Raycaster and Debug.Drawline
- [] [Raycasters](https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/70) 


Build Triangle in Mesh
- [Mesh](https://github.com/EloiStree/HelloUnityKeywordForJunior/issues/67)






----------

To Refactor
-https://github.com/EloiStree/2019_08_06_InClassCodeHelloTechnifutur/tree/master/Assets


Lost
![image](https://github.com/user-attachments/assets/2423bf3e-3521-4d59-9537-e3a00881db81)
2019_08_20_HandToTankGun

